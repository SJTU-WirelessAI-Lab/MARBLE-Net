...
def visualize_beam_patterns(model, fm_list, Nt, d, save_dir, tag):
    ...
    2. Beam pointing angle histogram (beam squint distribution)
    """
    model.eval()
...
class RainbowBeamModel(nn.Module):
    ...
    def forward(self, H, fm_list, pt_scaling_factor, noise_std_dev):
        ...
        # Fix: increase epsilon from 1e-20 to 1e-10 to prevent gradient explosion at deep nulls
...
class MARBLE(nn.Module):
    def __init__(self, input_len: int):
        ...
        self.features = nn.Sequential(
            nn.Conv1d(1, 64, 7, 2, 3), nn.BatchNorm1d(64), nn.LeakyReLU(0.1), nn.Dropout(0.1),
            nn.Conv1d(64, 128, 7, 2, 3), nn.BatchNorm1d(128), nn.LeakyReLU(0.1), nn.Dropout(0.1),
            nn.Conv1d(128, 256, 7, 2, 3), nn.BatchNorm1d(256), nn.LeakyReLU(0.1), nn.Dropout(0.1),
            nn.Conv1d(256, 256, 3, 1, 1), nn.BatchNorm1d(256), nn.LeakyReLU(0.1), nn.Dropout(0.1),
            nn.Conv1d(256, 256, 3, 1, 1), nn.BatchNorm1d(256), nn.LeakyReLU(0.1), nn.Dropout(0.1),
        )
        with torch.no_grad(): flat_dim = self.features(torch.zeros(1, 1, input_len)).numel()
        self.classifier = nn.Sequential(
            nn.Linear(flat_dim, 256), nn.BatchNorm1d(256), nn.LeakyReLU(0.1),
            nn.Linear(256, 128), nn.BatchNorm1d(128), nn.LeakyReLU(0.1),
            nn.Linear(128, 84), nn.BatchNorm1d(84), nn.LeakyReLU(0.1),
            nn.Linear(84, 2), nn.Tanh(),
        )
    def forward(self, x):
        z = self.features(x.to(torch.float32).unsqueeze(1))
    ...
    for epoch in range(epochs):
        ...
        for batch_data in train_loader:
            ...
            if training_objective == 'energy':
                ...
                # mag_sq_db is in dB domain; convert back to linear, sum, then take negative dB as Loss
                ...
                # Placeholder: do not compute localization errors in energy mode
                ...
                # [Norm] Normalize dB values to approx [-1,1] to avoid gradient instability from large magnitudes
            ...
            # Gradient clipping (prevent exploding gradients in strong LoS scenarios).
            # Apply clipping to both Model 1 and Model 2 parameters with max_norm=1.0
            if train_model1:
        ...
        if epochs_no_improve >= patience:
            print(f"\nEarly stopping triggered: {session_name} did not improve for {patience} epochs. Ending training session.")
            break
...
def evaluate_and_plot(model1, model2, test_loader, device, fm_list, pt_scaling_factor, noise_std_dev, dis_max, args, final_session_name):
    print("\n" + "="*60)
    print(f"Starting final evaluation (based on {final_session_name})...")
    print("="*60 + "\n")
...
def analyze_param_change(model, prev_params, stage_name):
    """Analyze model parameters (PS, TTD) changes and print statistics.
    """
    ...
    print(f"Range scatter saved to: {range_fig_path}")
    plt.close()
    ...
    print(f"Angle scatter saved to: {angle_fig_path}")
    plt.close()



def recalibrate_bn(model, loader, device, fm_list, pt_scaling_factor, noise_std_dev, model1_frozen=None):
    """Run a forward pass over a subset of data to update BatchNorm running statistics.

    When the Beamformer (Model 1) is updated, the input distribution to the Receiver
    (Model 2) can change. This helper updates BN running_mean and running_var so that
    subsequent training/evaluation does not suffer from distribution shift spikes.
    """
    model.train()  # set train mode to update BN statistics
    print("Recalibrating BatchNorm statistics...")
    
    with torch.no_grad():  # no backward needed
        # If calibrating Model 2, Model 1 must provide inputs
        if model1_frozen is not None:
             model1_frozen.eval()

        for i, batch_data in enumerate(loader):
            # Run only a subset (e.g., 100 batches) to obtain stable BN statistics
            if i > 100: break
            
            # [Fix] Use same data unpacking as main loop (take first value as H)
            ...
            if model1_frozen is not None:
                ...
                # Model 2 forward pass (only to update stats)
                # [Norm] Must be consistent with training
                _ = model(mag_sq_db / 100.0)
            else:
                # Assume Model 1 self-calibration (Model 1 has no BN but keep logic)
...
def main():
    ...
    print("    3. Stabilization: log epsilon increase (1e-20 -> 1e-10)")
    print("    4. [Optimization] Stage 2 aggressive LR decay (LR / 5 for Cycle > 1)")
    print("    5. [New] Input normalization (Input / 100.0) -> mitigates baseline oscillation")
    print("="*60)
    
    # --- 1. Parameters and paths ---
    parser = argparse.ArgumentParser(description='Train the MARBLE model for position estimation.')
    ...
    parser.add_argument('--lr_model2', type=float, default=1e-3, help='Learning rate for model2 (MARBLE).')
    ...
    print(f"Noise std (scaled): {scaled_noise_std:.4e} (raw: {raw_noise_std:.4e})")

    # --- 4. Build models ---
    # [Modified] Use wide-range initialization (-60 ~ 60) to allow energy maximization to auto-focus
    phi_init_start = -60.0
    phi_init_end = 60.0
    print(f"Beam initialization params: phi_start={phi_init_start}, phi_end={phi_init_end} (wide range)")
    ...
    model2 = MARBLE(input_len=num_subcarriers).to(device)

    print("\n" + "="*60)
    print("Model complexity:")
    print(f"  - Model 1 (RainbowBeamModel) Trainable Params: {count_parameters(model1)}")
    print(f"  - Model 2 (MARBLE) Trainable Params          : {count_parameters(model2)}")
    print("="*60)
    ...
    if args.training_mode == 'joint':
        ...
        # Iteration auto-control
        best_cycle_val_loss = float('inf')
        loss_threshold = 1e-5  # Threshold to decide if loss has decreased
        max_cycles = 20        # Set a large max cycles to avoid infinite loop
        
        print(f"Iteration mode: auto-stop enabled (Max Cycles: {max_cycles}, Threshold: {loss_threshold})")
        
        # Initial evaluation and visualization
        print("\n" + "="*40)
        ...
        # ==========================================
        # Resume Check & Baseline Comparison
        # ==========================================
        ...
        if os.path.exists(resume_c1_path):
            ...
            print(">>> Resume: Skipping baseline and continuing iteration loop.")
            print("="*60)
        else:
            # Baseline comparison: run a baseline where beamformer is fixed and only receiver is trained
            print("\n" + "="*60)
            print(">>> Baseline: starting baseline test (freeze initial beamformer, train receiver only)")
            print("="*60)

            # Save initial weights so we can restore them after baseline
            ...
            print(f"\n>>> Baseline complete: performance with fixed initial beamformer:")
            ...
            # Restore initial models and continue with iteration training
            print("\n" + "="*60)
            print(">>> Reset: restoring models to initial state, starting iteration training...")
            print("="*60)
        ...
        for i in range(max_cycles):
            cycle_num = i + 1
            print("\n" + "#"*70)
            print(f"# Starting iteration: Cycle {cycle_num} (Max: {max_cycles})")
            print("#"*70 + "\n")
            
            # Resume logic: check if this cycle is already completed
            ...
            if i == 0:
                # === First cycle special handling: use a three-step strategy to maximize energy (mirrors log1214_total.txt) ===
                ...
                # Use a smaller learning rate for fine-tuning (ref log1214: lr=0.001)
                ...
                # Joint fine-tune usually doesn't need many epochs; set to 50 here (ref log1214)
                ...
                session_p1 = session_p1_3 # mark as latest session
                
                # Visualization: after Stage 1 energy training, generate beam plots
                print(f">>> Generating plots for Cycle 1 Energy Stage...")
            ...
            # [Eval] After Stage 1, evaluate Beamformer's impact on localization (MARBLE unchanged)
            print(f"\n>>> [Cycle {cycle_num} Stage 1 Complete] Evaluate system performance after Beamformer update (MARBLE not updated):")
            ...
            # BN recalibration: After Beamformer update, Model 2 input distribution may change.
            # Calibrate BN stats before evaluation/training to reduce loss spikes.
            ...
            # --- Stage 2: Train MARBLE (Model 2) ---
            session_p2 = f"ITER-C{cycle_num}-S2-NET-LOC"
            print(f">>> [Cycle {cycle_num}] Stage 2: Train MARBLE (objective: location)")
            
            # [Optimization] Use a mild exponential decay across cycles to avoid LR becoming too small
            # Strategy change: from direct /5.0 to *0.7 per cycle (exponential decay)
            decay_rate = 0.7
            current_lr_model2 = args.lr_model2 * (decay_rate ** (cycle_num - 1))
            
            # Set a lower bound to prevent lr approaching zero
            min_lr = 1e-5
            current_lr_model2 = max(current_lr_model2, min_lr)

            if cycle_num > 1:
                print(f"Auto-adjust: Cycle {cycle_num}, adjusting Model2 lr to {current_lr_model2:.1e} (decay: {decay_rate}^k)")
            ...
            print(f"\n>>> [Cycle {cycle_num} Complete] Current system performance evaluation:")
            ...
            if current_cycle_val_loss < best_cycle_val_loss - loss_threshold:
                print(f"Performance improved (Loss: {best_cycle_val_loss:.6f} -> {current_cycle_val_loss:.6f}), continuing to next cycle.")
                best_cycle_val_loss = current_cycle_val_loss
            else:
                print(f"No significant improvement (Diff < {loss_threshold}), stopping iterations.")
                break
        ...
        # Stage 1: Train Beamformer only, objective: maximize total energy
        ...
        # Stage 2: Fix Beamformer, train MARBLE, objective: localization
    ...
    if final_session_name:
        print("\n" + "="*60)
        print("All training completed. Loading final best models for testing and plotting...")
        ...
        if os.path.exists(best_model1_path) and os.path.exists(best_model2_path):
            ...
            # After final training, generate beam plots
        ...
        print("No training sessions run; skipping final evaluation.")
